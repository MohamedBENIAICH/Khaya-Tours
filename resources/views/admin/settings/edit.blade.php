        ...
        <div class="card shadow">
            ...
            <div class="card-body">
                <form action="{{ route('admin.settings.update', $setting->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        <img style="margin: 1rem" src="{{ Storage::url($setting->logo) }}" alt="">
                        <label for="logo">{{ __('logo') }}</label>
                        <input type="file" class="form-control" id="logo" placeholder="{{ __('logo') }}" name="logo" value="{{ old('logo' , $setting->logo) }}" />
                    </div>
